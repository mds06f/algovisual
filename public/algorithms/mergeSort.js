...
export const algorithm = {
  ...
  category: 'Sorting',
  description:
    'Merge Sort is a divide-and-conquer algorithm. It recursively splits the array into two halves, sorts each half, then merges them back together in order. An auxiliary canvas beneath the main view shows the active sub-array segments.',
  pseudocode: [
  ...
  generator: function (arr) {
    ...
    function mergeSort(start, end) {
      ...
      snap(
        1,
        `Splitting segment at indices [${start}..${end}] into [${start}..${mid}] and [${mid + 1}..${end}].`,
      ...
      snap(
        9,
        `Merging sub-arrays [${start}..${mid}] and [${mid + 1}..${end}].`,
        Array.from({ length: end - start + 1 }, (_, i) => start + i),
        [...L],
        [...R],
        start,
      ...
      while (i < L.length && j < R.length) {
        ...
        if (L[i] <= R[j]) {
          ...
          snap(
            12,
            `L[${i}]=${L[i]} ≤ R[${j}]=${R[j]}. Place ${L[i]} at index ${k}.`,
            [k],
            [...L],
            [...R],
            start,
          ...
          snap(
            13,
            `L[${i}]=${L[i]} > R[${j}]=${R[j]}. Place ${R[j]} at index ${k}.`,
            [k],
            [...L],
            [...R],
            start,
      ...
      while (i < L.length) {
        ...
        snap(
          14,
          `Copy remaining L[${i}]=${L[i]} to index ${k}.`,
          [k],
          [...L],
          [...R],
          start,
      ...
      while (j < R.length) {
        ...
        snap(
          14,
          `Copy remaining R[${j}]=${R[j]} to index ${k}.`,
          [k],
          [...L],
          [...R],
          start,
